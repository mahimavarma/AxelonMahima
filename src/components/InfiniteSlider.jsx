...
export default function BrandCarousel() {
  ...
  const renderBrandCard = (brand, index, keyPrefix) => {
    // Brands that need text beside the logo
    const needsTextBeside = ["Mail Mafia", "QuickNexus"];
    
    if (needsTextBeside.includes(brand.name)) {
      return (
        <div key={`${keyPrefix}-${index}`} className="flex-shrink-0 flex items-center justify-center w-[200px] sm:w-[220px] md:w-[250px] h-[80px] sm:h-[90px] md:h-[100px]">
          {imageErrors[`logo-${index}`] ? (
            ...
            <div className="flex items-center gap-3 w-full justify-center">
              <img
                ...
                className="w-[50px] sm:w-[60px] md:w-[70px] h-[50px] sm:h-[60px] md:h-[70px] object-contain hover:opacity-90 transition-opacity duration-300"
                style={{
        ...
        {imageErrors[`logo-${index}`] ? (
          ...
          <img
            ...
            style={{
              ...
              mixBlendMode: 'normal',
              transform: brand.name === "Venera Biotech" ? 'scale(0.7)' : 'none'
            }}
          />
        )}
      </div>
